#!/usr/bin/env python3
"""Generate route maps offline using matplotlib with proper Chinese font support."""

import matplotlib
matplotlib.use('Agg')
from matplotlib import font_manager
import matplotlib.pyplot as plt
from matplotlib.font_manager import FontProperties
import matplotlib.patches as mpatches
import numpy as np
import os

# Load Chinese font directly
ZH_FONT = FontProperties(fname='/usr/share/fonts/truetype/wqy/wqy-microhei.ttc')

# ── Route coordinate data ──

kai_tak_routes = {
    "Route A - 启德核心区闭环 (~4.5km)": {
        "color": "#e74c3c",
        "coords": [(22.3330, 114.1960), (22.3325, 114.2000), (22.3318, 114.2040),
                   (22.3290, 114.2070), (22.3260, 114.2090), (22.3250, 114.2080),
                   (22.3240, 114.2065), (22.3235, 114.2045), (22.3260, 114.2020),
                   (22.3300, 114.1990), (22.3330, 114.1960)],
        "roads": ["协调道", "承启道", "沐安街", "沐元街", "启成街"],
    },
    "Route B - 跨区综合路线 (~7km)": {
        "color": "#2ecc71",
        "coords": [(22.3330, 114.1960), (22.3325, 114.2000), (22.3318, 114.2040),
                   (22.3260, 114.2090), (22.3240, 114.2120), (22.3220, 114.2150),
                   (22.3190, 114.2180), (22.3180, 114.2160), (22.3170, 114.2140),
                   (22.3160, 114.2120), (22.3140, 114.2100), (22.3120, 114.2080),
                   (22.3180, 114.2160), (22.3200, 114.2130), (22.3220, 114.2100),
                   (22.3240, 114.2090), (22.3330, 114.1960)],
        "roads": ["协调道", "承丰道", "观塘道", "宏光道", "宏照道", "启祥道", "常悦道"],
    },
    "Route C - 启德邮轮码头特色路线 (~5.5km)": {
        "color": "#3498db",
        "coords": [(22.3330, 114.1960), (22.3325, 114.2000), (22.3318, 114.2040),
                   (22.3260, 114.2090), (22.3240, 114.2120), (22.3220, 114.2150),
                   (22.3210, 114.2180), (22.3200, 114.2200), (22.3260, 114.2090),
                   (22.3250, 114.2080), (22.3240, 114.2065), (22.3330, 114.1960)],
        "roads": ["协调道", "承启道", "承丰道", "邮轮码头路", "沐安街"],
    },
}

shatin_routes = {
    "Route D - 科学园环线 (~3.5km)": {
        "color": "#e67e22",
        "coords": [(22.4260, 114.2100), (22.4270, 114.2120), (22.4280, 114.2140),
                   (22.4290, 114.2180), (22.4280, 114.2160), (22.4270, 114.2140),
                   (22.4260, 114.2100)],
        "roads": ["科学园路", "科技大道西", "创新路", "瑞祥街"],
    },
    "Route E - 校园-科学园联动路线 (~6km)": {
        "color": "#9b59b6",
        "coords": [(22.4180, 114.2070), (22.4190, 114.2085), (22.4200, 114.2100),
                   (22.4230, 114.2120), (22.4260, 114.2100), (22.4280, 114.2140),
                   (22.4290, 114.2180), (22.4310, 114.2180), (22.4330, 114.2220),
                   (22.4340, 114.2250), (22.4180, 114.2070)],
        "roads": ["大学路", "吐露港公路", "科学园路", "科技大道西", "马鞍山绕道"],
    },
    "Route F - 吐露港走廊快速路线 (~8km)": {
        "color": "#1abc9c",
        "coords": [(22.4260, 114.2100), (22.4270, 114.2120), (22.4280, 114.2140),
                   (22.4230, 114.2120), (22.4310, 114.2180), (22.4320, 114.2200),
                   (22.4330, 114.2220), (22.4310, 114.2180), (22.4280, 114.2140)],
        "roads": ["科学园路", "吐露港公路(南行)", "大老山公路", "马鞍山绕道"],
    },
}

LANDMARKS_KE = {
    "启德发展区": (22.328, 114.203),
    "邮轮码头": (22.320, 114.220),
    "九龙湾": (22.318, 114.215),
    "启德跑道公园": (22.323, 114.212),
}

LANDMARKS_ST = {
    "香港科学园": (22.428, 114.214),
    "香港中文大学": (22.419, 114.208),
    "马鞍山": (22.433, 114.223),
}


def draw_map(routes, landmarks, title, filename, figsize=(20, 14)):
    """Draw a route map with labels and legend."""
    fig, ax = plt.subplots(1, 1, figsize=figsize)

    # Collect all coordinates
    all_lat = []
    all_lon = []
    for info in routes.values():
        for lat, lon in info["coords"]:
            all_lat.append(lat)
            all_lon.append(lon)

    margin_lat = (max(all_lat) - min(all_lat)) * 0.15
    margin_lon = (max(all_lon) - min(all_lon)) * 0.15
    ax.set_xlim(min(all_lon) - margin_lon, max(all_lon) + margin_lon)
    ax.set_ylim(min(all_lat) - margin_lat, max(all_lat) + margin_lat)

    ax.grid(True, linestyle='--', alpha=0.3, color='#888888')
    ax.set_facecolor('#f0f8ff')

    # Draw routes
    for name, info in routes.items():
        lats = [c[0] for c in info["coords"]]
        lons = [c[1] for c in info["coords"]]

        ax.plot(lons, lats, color=info["color"], linewidth=3, alpha=0.8,
                marker='o', markersize=6, label=name)

        # Road labels
        coords = info["coords"]
        roads = info["roads"]
        for i, road in enumerate(roads):
            if i < len(coords):
                lat, lon = coords[i]
                ax.text(lon, lat, road, fontsize=9, color=info["color"],
                        fontweight='bold', ha='center', va='bottom',
                        fontproperties=ZH_FONT,
                        bbox=dict(boxstyle='round,pad=0.2', facecolor='white',
                                 edgecolor=info["color"], alpha=0.7))

        # Start/end
        start = coords[0]
        end = coords[-1]
        ax.text(start[1], start[0], '▶ 起点', fontsize=11, color='green',
                fontweight='bold', ha='left', va='bottom', fontproperties=ZH_FONT,
                bbox=dict(boxstyle='round,pad=0.3', facecolor='#d5f5e3', edgecolor='green'))
        ax.text(end[1], end[0], '⏹ 终点', fontsize=11, color='red',
                fontweight='bold', ha='left', va='top', fontproperties=ZH_FONT,
                bbox=dict(boxstyle='round,pad=0.3', facecolor='#fadbd8', edgecolor='red'))

    # Landmarks
    for name, (lat, lon) in landmarks.items():
        ax.text(lon, lat, ' ' + name, fontsize=10, color='#555',
                ha='center', va='center', fontproperties=ZH_FONT,
                bbox=dict(boxstyle='circle,pad=0.3', facecolor='#fff3cd',
                         edgecolor='#f0ad4e', alpha=0.8))

    # Legend
    legend_elements = [mpatches.Patch(color=info["color"], label=name)
                       for name, info in routes.items()]
    ax.legend(handles=legend_elements, loc='upper left', fontsize=11,
              framealpha=0.9, fancybox=True, prop=ZH_FONT)

    ax.set_title(title, fontsize=18, fontweight='bold', pad=20, fontproperties=ZH_FONT)
    ax.set_xlabel('经度 (Longitude)', fontsize=12, fontproperties=ZH_FONT)
    ax.set_ylabel('纬度 (Latitude)', fontsize=12, fontproperties=ZH_FONT)

    plt.tight_layout()
    plt.savefig(filename, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"✅ {filename}")


out_dir = "/root/.openclaw/workspace/tools"

draw_map(kai_tak_routes, LANDMARKS_KE,
         "九龙东 (Kowloon East) - 自动驾驶 Demo 路线",
         f"{out_dir}/routes_kowloon_east.png")

draw_map(shatin_routes, LANDMARKS_ST,
         "沙田 (Shatin) - 自动驾驶 Demo 路线",
         f"{out_dir}/routes_shatin.png")

all_routes = {**kai_tak_routes, **shatin_routes}
all_landmarks = {**LANDMARKS_KE, **LANDMARKS_ST}
draw_map(all_routes, all_landmarks,
         "全部自动驾驶 Demo 路线总览 (九龙东 + 沙田)",
         f"{out_dir}/routes_all_combined.png", figsize=(24, 16))

print("\n🎉 All route maps generated!")
