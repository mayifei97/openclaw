// Evolution Strategy Presets (v1.2)
...
//
// === v1.2 Enhancements ===
// - Added consecutive failure detection for forced innovation switch
// - Lowered repairLoopThreshold for faster recovery from stuck states
...
function _readRecentFailureCount() {
  // Read recent evolution events to count consecutive failures
  // Returns the number of consecutive failed cycles (0 if last was success or no data)
  try {
    var localPath = path.resolve(__dirname, '..', '..', 'memory', 'evolution', 'evolution_state.json');
    var workspacePath = path.resolve(__dirname, '..', '..', '..', '..', 'memory', 'evolution', 'evolution_state.json');
    var eventsPath = null;
    
    // Check which evolution dir exists
    var candidates = [
      path.resolve(__dirname, '..', '..', 'memory', 'evolution'),
      path.resolve(__dirname, '..', '..', '..', '..', 'memory', 'evolution'),
    ];
    
    for (var i = 0; i < candidates.length; i++) {
      if (fs.existsSync(candidates[i])) {
        eventsPath = path.join(candidates[i], 'events.jsonl');
        break;
      }
    }
    
    if (!eventsPath || !fs.existsSync(eventsPath)) return 0;
    
    var raw = fs.readFileSync(eventsPath, 'utf8');
    var lines = raw.split('\n').map(function(l) { return l.trim(); }).filter(Boolean);
    
    // Read last 10 events, count consecutive failures from the most recent
    var recentLines = lines.slice(Math.max(0, lines.length - 10));
    var consecutiveFailures = 0;
    
    for (var j = recentLines.length - 1; j >= 0; j--) {
      try {
        var event = JSON.parse(recentLines[j]);
        if (event.kind === 'outcome' && event.outcome) {
          if (event.outcome.status === 'failed' || event.outcome.status === 'error_persisted') {
            consecutiveFailures++;
...
function resolveStrategy(opts) {
  ...
  if (isDefault) {
    ...
    // === v1.2 Enhancement: Consecutive failure detection ===
    // If 2+ consecutive failures detected, force innovation mode to break the loop
    var consecutiveFailures = _readRecentFailureCount();
    if (consecutiveFailures >= 2) {
      console.log('[Strategy] Detected ' + consecutiveFailures + ' consecutive failures, forcing innovation mode');
      name = 'innovate';
    }
    ...
    // Add signal for prompt to be aware of failure recovery mode
    if (consecutiveFailures >= 1 && consecutiveFailures < 2) {
      signals.push('consecutive_failure_warning:' + consecutiveFailures);
    }
  }
