...
function getMemoryAdvice({ signals, genes, driftEnabled }) {
  ...
  for (const [geneId, info] of byGene.entries()) {
    ...
    // === Enhanced failure detection (v1.2) ===
    // Aggressive ban on repeated failures to avoid "repairing already-fixed problems"
    
    // Rule 1: Ban after 2 consecutive failures with low score (lowered threshold from 0.18 to 0.25)
    if (!driftEnabled && info.attempts >= 2 && info.best < 0.25) {
      bannedGeneIds.add(geneId);
    }
    
    // Rule 2: Ban after 1 failure if the gene has poor global outcomes (faster reaction)
    if (!driftEnabled && info.attempts >= 1 && info.best < 0.15 && info.prior_attempts >= 2 && info.prior < 0.20) {
      bannedGeneIds.add(geneId);
    }
    
    // Rule 3: Also suppress genes with consistently poor global outcomes when signal edges are sparse.
