...
async function run() {
  ...
  // === v1.2 Enhancement: Problem Verification Before Repair ===
  // If the selected gene is a repair gene, verify that the error signals still exist.
  // This prevents "repairing already-fixed problems" which wastes cycles.
  if (selectedGene && selectedGene.category === 'repair') {
    const errorSignals = signals.filter(function(s) {
      var str = String(s || '').toLowerCase();
      return str.includes('log_error') || str.includes('errsig:') || str.includes('error') || str.includes('failed');
    });
    
    if (errorSignals.length > 0) {
      // Check if these error signals appear in recent logs (last 500 lines)
      var recentLogPath = path.join(getRepoRoot(), '..', '..', 'logs', 'openclaw-' + new Date().toISOString().split('T')[0] + '.log');
      var altLogPath = '/tmp/openclaw/openclaw-' + new Date().toISOString().split('T')[0] + '.log';
      var logToCheck = fs.existsSync(recentLogPath) ? recentLogPath : (fs.existsSync(altLogPath) ? altLogPath : null);
      
      var errorsStillExist = false;
      if (logToCheck && fs.existsSync(logToCheck)) {
        try {
          var logContent = fs.readFileSync(logToCheck, 'utf8');
          var recentLines = logContent.split('\n').slice(-500).join('\n');
          
          // Check if any error signal pattern appears in recent logs
          for (var ei = 0; ei < errorSignals.length; ei++) {
            var errSig = String(errorSignals[ei] || '');
            if (errSig.startsWith('errsig:')) {
              var errPattern = errSig.slice('errsig:'.length).toLowerCase().slice(0, 50);
              if (errPattern && recentLines.toLowerCase().includes(errPattern)) {
                errorsStillExist = true;
                break;
              }
            } else if (recentLines.toLowerCase().includes(errSig.toLowerCase())) {
              errorsStillExist = true;
              break;
            }
          }
        } catch (e) {
          // Log read failure is non-fatal, proceed with caution
          console.log('[Evolver] Log verification skipped: ' + e.message);
          errorsStillExist = true; // Assume errors exist if we can't verify
        }
      } else {
        // No log file available, assume errors exist (fail open)
        errorsStillExist = true;
      }
      
      if (!errorsStillExist) {
        console.log('[Evolver] Repair gene selected but error signals not found in recent logs. Skipping repair.');
        console.log('[Evolver] Adding force_innovation signal to avoid wasted repair cycle.');
        signals.push('force_innovation');
        signals.push('problem_already_resolved');
      }
    }
  }
  ...
  try {
    ...
    // Based on historical data analysis (2026-03-11):
    // - Average actual/estimated ratio: 8.0%
    // - Historical pattern: estimates are ~12.5x larger than actual
    // Calibration factor reduces over-estimation while keeping safety margin
    const CALIBRATION_FACTOR = 0.15; // Calibrated from 0.08 historical average + safety margin
    const LINES_PER_FILE_AVG = 60;   // Calibrated from historical ~42-49 lines per file

    const calibratedFiles = Math.max(1, Math.round(maxFiles * CALIBRATION_FACTOR));
    const calibratedLines = Math.round(calibratedFiles * LINES_PER_FILE_AVG);
    ...
    const blastRadiusEstimate = {
      ...
      raw_estimate: {
        files: maxFiles,
        lines: Math.round(maxFiles * 80),
      },
      calibration: {
        factor: CALIBRATION_FACTOR,
        lines_per_file: LINES_PER_FILE_AVG,
        based_on: 'historical_events_analysis_2026-03-11',
      },
    };
